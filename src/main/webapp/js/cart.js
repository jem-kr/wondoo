...
function plusqty(){
	...
	if(cart_cust_qty >= pro_stock) { // 재고를 초과한 경우
		...
		$('#cart_cust_qty').val(cart_cust_qty);
		$('#cart_total_price').text(cart_cust_qty * pro_price);
	}else{ // 재고를 초과하지 않은 경우
		cart_cust_qty += 1;
...
function minusqty(){
	...
	if(cart_cust_qty <= 1) { // 사용자가 지정한 재고가 1인 경우
		cart_cust_qty -= 0;
