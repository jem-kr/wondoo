...
public class SellPWSearchController extends SuperClass {
	...
	private final String redirect = "redirect:/sellPWResult.se";
	...
	public SellPWSearchController() {
		super("sellPWSearch", null);
	...
	public ModelAndView doPost(
			...
			@RequestParam(value = "sell_Name", required = false) String sell_Name,
			@RequestParam(value = "sell_Contact", required = false) String sell_Contact,
			HttpSession session) {
		...
		if (isCheck == true) {
			Seller bean = this.sdao.SelectPW(sell_Email, sell_Name, sell_Contact);
			if (bean == null) { 
