...
public class CustIdSearchController extends SuperClass {
	...
	public ModelAndView doPost(
		...
		if (isCheck == true) {
			// 이름, 연락처, 생년월일 등등 3개의 데이터로 일치하는 회원 정보를 조회합니다.
			Customer bean = this.cdao.SelectEmail(cust_Name, cust_Contact, cust_Birth);
			if (bean == null) { 
				System.out.println("일치하는 회원정보가 없음");
				this.mav.setViewName("redirect:/custIdSearch.cu");
